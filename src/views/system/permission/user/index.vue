...
<script setup lang="tsx">
  ...
  const openUserModal = async (record: Partial<TableListItem> = {}) => {
    ...
    const [formRef] = await showModal<any>({
      modalProps: {
        ...
        onFinish: async (values) => {
          ...
          await (record.userid ? updateUser : createUser)(values);
    ...
    formRef?.updateSchema([
      {
        field: 'orgid',
        componentProps: {
          treeDefaultExpandedKeys: findChildById(record?.orgid, state.deptTree)?.keyPath || [],
    ...
    if (record?.userid) {
      const { roles } = await getUserInfo({ userId: record.userid });
  ...
  type UserLoadDataParams = API.UserPageParams & LoadDataParams;

  const loadTableData = async ({ page, limit, loginid, username, phone }: UserLoadDataParams) => {
